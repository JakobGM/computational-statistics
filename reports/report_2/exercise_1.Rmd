...
We recognize $\sum_{i=1}^{n}(y_i\eta_i - E_i\exp(\eta_i))$ as $\sum_{i=1}^nf(\eta_i)$. By substituting $f(\eta_i)$ for it's approximation $\tilde{f}(\eta_i)$ we get $\sum_{i=1}^n\tilde{f}(\eta_i) = \sum_{i=1}^na_i + \etavec^T\boldsymbol{b} - \frac{1}{2}\etavec^T\textrm{diag}(\boldsymbol{c})\etavec$

This enables us to find the approximation the following approximation of $p(\etavec \mid \uvec, \yvec, \kappa_u, \kappa_v)$.

\begin{align*}
q(\etavec \mid z, \uvec, \yvec, \kappa_u, \kappa_v) &= \exp\left(\etavec^T\boldsymbol{b} + \frac{1}{2}{\etavec}^T\textrm{diag}(\boldsymbol{c})\etavec - \frac{\kappa_v}{2}(\etavec - \uvec)^T\kappa_v\boldsymbol{I}(\etavec - \uvec) \right) \\

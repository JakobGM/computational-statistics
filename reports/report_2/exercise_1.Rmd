...
The posterior distribution $p(\etavec, \uvec, \kappa_u, \kappa_v \mid \yvec)$ can be found by combining Bayes' theorem and the definition of conditional probability with the distributions given in the problem description. We get that
...
\begin{align*}
p(\etavec, \uvec, \kappa_u, \kappa_v \mid \yvec)
&\propto p(\yvec \mid \etavec, \uvec, \kappa_u, \kappa_v)p(\etavec, \uvec, \kappa_v)p(\kappa_u\mid\etavec, \uvec, \kappa_v) \\
...
\begin{align*}
p(\etavec, \uvec, \kappa_u, \kappa_v \mid \yvec)
&\propto p(\yvec\mid\etavec)p(\etavec\mid\uvec,\kappa_v)p(\kappa_v)p(\uvec\mid\kappa_u)p(\kappa_u)
\end{align*}

Inserting the expressions for the probability distributions, combined with 

\begin{align*}
p(\yvec\mid\etavec) &= \prod_{i=1}^n\frac{(E_i\exp(\eta_i))^{y_i}}{y_i!}\exp({-E_i\exp(\eta_i)}) \propto \exp(\sum_{i=1}^ny_i\log(E_i) + y_i\eta_i - E_i\exp(\eta_i)) \\ 

...
\newpage
As we can see in figure \ref{fig:acf}, $\kappa_v$ has the greatest degree of autocorrelation, as predicted from the trace plots in figure \ref{fig:burnin} earlier, while $\kappa_u$ has comparatively much less autocorrelation.
The three $\boldsymbol{v}$ components, i.e. unstructured random noise, have the lowest degree of autocorrelation, also as predicted from the previous plots.
...
As can be seen in figure \ref{fig:p_values_plot}, it is not probable that these $p$-values are *not* realizations of a uniform distribution, which indicates convergence.

...
```{r, fig.width = 8.3, fig.height = 8, fig.fullwidth=TRUE}
...
samples %>%
  ...
  facet_wrap(vars(key), scales = "free_y", ncol = 2) +
  labs(
    title = "Parameter burn-in trace plots",
    caption = "Figure 3-2: Sample line plot for beginning steps of the MCMC algorithm (burn-in). The red line indicates the chosen burn-in boundary."
  )
```

From figure `3-2` it becomes clear that all the 8 chains have converged long before a thousand steps (red vertical line) have been calculated.
...
We can also observe from this plot that the $\kappa_v$ sample plot is the most "sticky" of them all, and will probably show a greater degree of auto-correlation, analysis of which will be coming up in the next section.

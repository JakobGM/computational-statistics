...
This can be considered to be a good result, since a sufficient amount of proposal samples are expected to be accepted.
...
$\kappa_u$ starts off at $\simeq 25~000$ but has quickly started taking on a values in a more sensible region. It can be seen in table \ref{tab:posterior_means} that $\kappa_u$ ends up with a posterior mean of $\sim 17$.
...
The results in table \ref{tab:geweke} are promisising, as not a single null hypothesis needs to be discarded even at $10\%$ significance.
...
Table \ref{tab:ESS} tells us that it is estimated that independent samplers for $\kappa_u$ and $\kappa_v$ would only require $1156$ and $626$ samples, respectively, in order to obtain parameter estimates with the *same* precision as if we used our MCMC samples of size $70~000$. This is less than $2\%$ of the MCMC sample size, $M$.

...
high_exponents <- function(base, exponent){
  remainder <- exponent - floor(exponent)
  prime_factors <- primeFactors(floor(exponent))
  result = base
...
rou_rf <- function(n, rate){
  a <- high_exponents((rate - 1)/exp(1), (rate - 1)/2)
  b_plus <- high_exponents((rate + 1)/exp(1), (rate + 1)/2)
  ...
  random_samples <- random_samples %>%
    mutate(upperBound = ((rate - 1)*(log(x2) - log(x1)) - x2/x1)/2) %>%
...
required_trials <- function(rate){
  ...
  while (accepted_samples < 1000){
    ...
    x1 <- runif(1)*a
    x2 <- runif(1)*b_plus
    if(log(x1) <= ((rate - 1)*(log(x2) - log(x1)) - x2/x1)/2){
...
trials_df <- tibble(rate=seq(3,300, 5), trials = seq(3,300, 5) %>% map_dbl(~required_trials(.x)))
trials_df %>%

...
$$
\newcommand{c}[0]{\frac{\alpha e}{\alpha + e}}
g(x) =
\begin{cases}
  \c x^{\alpha - 1}, & 0 < x < 1, \\
  \c e^{-x}, & 1 \leq x, \\
...
```
```{r}
c <- rate*exp(1)/(rate + exp(1))
mean <- c*(1/(rate + 1) + 2/exp(1))
second_moment <- c*(1/(rate+2) + 5/exp(1))
...
std_normal_dist <- as_mapper(~1/sqrt(2*pi)*exp(-.x^2/2))
n <- 1000000
x <- box_muller(n)
...
test_plot <- sample %>%
  ...
  stat_function(fun=std_normal_dist, color="red", size=1)

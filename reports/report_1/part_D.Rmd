...
```{r}
posteriorMean <- function(theta_samples) {
  weights <- (1 - theta_samples$value) ** 4
  importanceThetaMean <- sum(theta_samples$value * weights) / sum(weights)
  return(importanceThetaMean)
}
importanceThetaMean <- posteriorMean(theta_samples = theta_samples)
...
posterior <- generatePosterior(y = y)
posteriorIntegralMean <- integrate(
  f = function(theta) theta * posterior(theta),
  lower = 0,
  upper = 1
)$value

meanResults <- tibble(
  posteriorMeans=c(importanceThetaMean, posteriorIntegralMean),
  grp=c("importance sampling", "numerical integration")
)
...
ggplot(tibble(theta = c(0, 1)), aes(theta)) +
  ...
  xlim(0, 1) +
  scale_color_manual(" ", values = c("red", "blue"), labels = c("importance sampling", "numerical integration"))
```

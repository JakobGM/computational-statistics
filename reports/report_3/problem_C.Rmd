...
```{r}
score <- function (lambda, data, indeces) {
  sum(indeces) / lambda + sum(data$z[!indeces] / expm1(lambda * data$z[!indeces])) - sum(data$z[indeces])
}
```

```{r}
lambda_mle <- list(
  lambda_0 = uniroot(f = score, interval = c(1e-3, 100), data = data, indeces = data$u)$root,
  lambda_1 = uniroot(f = score, interval = c(1e-3, 100), data = data, indeces = !data$u)$root
)
print(lambda_mle)
```

...
$$
We proceed by obtaining $B = 1500$ bootstrap samples of the data sequence for each of the estimators $\vec{\beta}_{LS}$ and $\vec{\beta}_{LS}$. Each bootstrap sample is obtained by first picking a random consecutive sequence of length 2 from the original data sequence $\vec{x}$ and calculating the remaining $T - 2$ data points by using the relation 

$$
x_t = \hat{\beta}_1x_{t-1} + \hat{\beta}_2x_{t-2} + \hat{\epsilon},
$$
where $\epsilon$ is chosen randomly from either the set $\{\hat{\epsilon}_3^{LS}, \ldots, \hat{\epsilon}_T^{LS}\}$ or $\{\hat{\epsilon}^{LA}_3, \ldots, \hat{\epsilon}_T^{LA}\}$. 

For each of the $B$ resampled data sequences we estimate $\vec{\beta}_{LS}$ and $\vec{\beta}_{LA}$. A histogram showing the obtained values is shown in Figure $\ref{fig:1}$ and $\ref{fig:2}$.

```{r, fig.cap=c("\\label{fig:1} Histogram of $\\vec{\\beta}_{LS}$ estimated from each of the $B$ bootstrap samples", "\\label{fig:2} Histogram of $\\vec{\\beta}_{LA}$ estimated from each of the $B$ bootstrap samples")}
...
```{r, fig.caption="\\label{fig:pred_hist_ls} Histogram of $x_{101}$ predictions using $\\vec{\\beta}_{LS}^{\\star}$"}
...
```{r, fig.caption="\\label{fig:pred_hist_la} Histogram of $x_{101}$ predictions using $\\vec{\\beta}_{LA}^{\\star}$"}

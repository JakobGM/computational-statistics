...
where $e_t$ are iid random variables with zero mean and constant variance, i.e. a non-Gaussian time-series.
Our sequence is of length $T = 100$.
...
For each of the B resampled data sequences we estimate $\vec{\beta}_{LS}$ and $\vec{\beta}_{LA}$. A histogram showing the obtained values are shown in Figure $\ref{fig:hist_ls}$ and $\ref{fig:hist_la}$.

The procedure explained above can be implemented as shown below.
...
The bias of the estimators can be estimated by

$$
\widehat{\text{bias}}_B = \hat{\beta}^{\star}(\cdot) - \hat{\beta}.
$$
The estimated bias for the two different estimators, $\hat{\vec{\beta}}_{LS}$ and $\hat{\vec{\beta}}_{LA}$, is shown in Table $\ref{tab:ls_bias}$ and $\ref{tab:la_bias}$.
...
$$
The estimated variance for the two estimators can be seen in Table $\ref{tab:ls_var}$ and $\ref{tab:la_var}$.
...
In the code below we implement the procedure for estimating the bias and variance of the estimators.
```{r}
la_bias <- list(
...
$$
Again, $\epsilon$ denotes a random sample from the corresponding set of centered estimated innovations. After obtaining the predictions we calculate the 2.5th and 97.5th percentiles in order to obtain a $95\%$ confidence interval. The resulting confidence intervals are shown in Table $\ref{tab:conf_int_ls}$ and $\ref{tab:conf_int_la}$. The histogram showing the distribution of the predictions can be seen in Figure $\ref{fig:pred_hist_ls}$ and $\ref{fig:pred_hist_la}$.

The code for obtaining the predictions and the corresponding confidence intervals follows below. 
